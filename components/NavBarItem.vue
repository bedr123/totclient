...
const componentClass = computed(() => {
  const base = [
    isDropdownActive.value
      ? `${store.state.style.navBarItemLabelActiveColorStyle} dark:text-slate-400`
      : `${store.state.style.navBarItemLabelStyle} dark:text-white dark:hover:text-slate-400 ${store.state.style.navBarItemLabelHoverStyle}`,
...
const itemLabel = computed(() =>
  props.item.isCurrentUser ? store.state.style.userName : props.item.label
);
